...
static HWND Createcmdlinewindow(void) {
  ...
  if (hwcmd==NULL) {                   // Window is not yet here, create new.
    ...
    hwbox=CreateWindow(
      ...
      hwcmd,(HMENU)ID_HWBOX,(HANDLE)Plugingetvalue(VAL_HINST),NULL);
    ...
    hwerr=CreateWindow(
      ...
      hwcmd,(HMENU)ID_HWERR,(HANDLE)Plugingetvalue(VAL_HINST),NULL);
...
extc int _export cdecl ODBG_Plugininit(
  ...
  Addtolist(0,0,"Command line plugin v%i.%02i",VERSIONHI,VERSIONLO);
...
extc void _export cdecl ODBG_Pluginaction(int origin,int action,void *item) {
  ...
  switch (action) {
    ...
    case 2:                            // "About", displays plugin info
      sprintf(s,"Command line plugin v%i.%02i\nWritten by Oleh Yuschuk",
        VERSIONHI,VERSIONLO);
      MessageBox(hwmain,s,"Command line",MB_OK|MB_ICONINFORMATION);
      break;
    default: break;
  };
};
